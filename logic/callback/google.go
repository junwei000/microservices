...
func (l *logic) GoogleCallback(ctx context.Context, code string) (*model.User, string, error) {
	...
	auth, err := l.repo.Authorize().GetByProvider(ctx, provider, providerID)
	...
	if auth != nil && auth.ID != 0 {
		...
		user, err = l.repo.User().GetByUid(ctx, auth.UserID)
		if err != nil {
			return nil, "", err
		}
		...
		_ = l.repo.Authorize().Update(ctx, auth.ID, authData)

		// Sync User Avatar if changed or empty
		if userInfo.Picture != "" && user.Avatar != userInfo.Picture {
			_ = l.repo.User().Update(ctx, user.ID, map[string]any{
		...
		user, err = l.repo.User().GetByEmail(ctx, userInfo.Email)
		if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, "", err
		}
		...
		if user == nil || user.ID == 0 {
			...
			if err := l.repo.User().Create(ctx, newUser); err != nil {
				return nil, "", err
			}
			user = newUser
		} else {
		...
		if err := l.repo.Authorize().Create(ctx, newAuth); err != nil {
			return nil, "", err
		}
	}
